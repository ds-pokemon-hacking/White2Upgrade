...
class LZSerializer(Serializer):
    def __init__(self):
        super().__init__()

    def _compress(self, input, windowclass=NLZ10Window):
    ...
    def compress(self, input, out):
        # header
        out.write(pack("<L", (len(input) << 8) + 0x10))

        # body
        length = 0
        for tokens in self.chunkit(self._compress(input), 8):
            flags = [type(t) == tuple for t in tokens]
            out.write(pack(">B", self.packflags(flags)))
    ...
    def compress_nlz11(self, input, out):
        # header
        out.write(pack("<L", (len(input) << 8) + 0x11))

        # body
        length = 0
        for tokens in self.chunkit(self._compress(input, windowclass=NLZ11Window), 8):
            flags = [type(t) == tuple for t in tokens]
            out.write(pack(">B", self.packflags(flags)))
            length += 1
    ...
    def dump_compress_nlz11(self, input, out):
        # body
        length = 0
        def dump():
            for t in self._compress(input, windowclass=NLZ11Window):
                if type(t) == tuple:
                    yield t
