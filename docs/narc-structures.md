...
### Battle NNS Authoring TOMLs

The battle graphics sources under `data/graphics/pokegra/<species>/` use concise
TOML authoring files for the G2D resources. Authors do not write `KBEC`,
`KNBA`, `KBCM`, `LBAL`, or `TXEU` sections directly. The file `format` selects
the generated container and section layout, while the serializer derives counts,
table offsets, frame property offsets, section headers, and file sizes from the
readable data.

`*.ncer.toml` contains named cell tables such as `[cells.head]`. Each cell
stores `objects = [...]`, where every object is an inline decoded OAM table.
Repeated object layouts are duplicated in the authoring file instead of
referenced through a separate object-group table. The generated `LBAL` labels
come from the cell table names in file order.

`*.nanr.toml` and `*.nmar.toml` contain named sequence tables such as
`[sequences.head]`. The generated `LBAL` labels come from the sequence table
names in file order. Each sequence stores `frames = [...]`, where every frame is
an inline table carrying its property data. Repeated properties are duplicated
in the authoring file instead of referenced by name. Any required alignment or
unused bytes after a property are stored as `padding_after` on that frame.
NANR frames reference NCER cells by name with `cell = "head"`. NMAR frames
reference NMCR multi-cells by name with `multi_cell = "CellAnime0"`.
Animation scale fields are authored as signed fixed-point decimal values:
`scale_w = 1.0` serializes to raw `0x00001000`, and `scale_w = -1.0`
serializes to raw `0xFFFFF000`.

`*.nmcr.toml` contains named multi-cell tables such as
`[multi_cells.multi_cell_0]`. Each multi-cell stores `properties = [...]`, where
each property row is an inline table that places its referenced animation
sequence. Properties reference NANR sequences by name with `sequence = "head"`.
Displayed-cell counts, property table offsets, and `LBAL` labels are derived.

Extended metadata is authored as a top-level `extended` field when the generated
file needs `TXEU`. Unknown fields that are not yet named by the public G2D
notes are kept as `unknown*`, but their surrounding table lengths and offsets
are still generated by the serializer.

Each species folder has an `nns.toml` manifest. The manifest order is the archive
order for that species, and G2D entries list direct source dependencies with
`depends_on`: NANR depends on the matching NCER, NMCR depends on NANR, and NMAR
depends on NANR plus NMCR. The builder validates that these referenced files
exist before serializing the entry.

### Pokemon Footprints

NARC: `a/1/6/5`

This archive contains the footprint graphics used by Pokemon summary/Dex style
screens. The game indexes this archive by file ID rather than by text label; for
expanded species, the summary path can request IDs beyond the original 655-file
archive. The expanded build stages files `165_00000000.bin` through
`165_00000831.bin` from `data/graphics/pokegra/footprints` directly into
`/a/1/6/5`, covering the current maximum species ID `826` plus the footprint
offset of `5`.
